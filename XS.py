...
fill_colors['PFD_R'] = 'deeppink'
fill_colors['UVol'] = 'dimgray'
fill_colors['Basement'] = 'green'
fill_colors['GroundSurface'] = 'black'
...
print z_interp['GroundSurface']
plt.ylim(np.ceil(z_min/10.)*10, np.ceil(np.nanmax(z_interp['GroundSurface'])/100.)*100)
...
for layerName in allData:
  values = allData[layerName]
  plt.plot(values['dist'], values['z'], '-', linewidth=2, label=layerName, color=fill_colors[layerName])
  plt.legend(loc='lower left')
"""
plt.savefig(os.path.splitext(file_name)[0]+'.svg')
plt.show()
